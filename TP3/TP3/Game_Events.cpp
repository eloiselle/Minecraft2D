		...
		if (Mouse::isButtonPressed(Mouse::Left))
		{
			...
			if (toolIsAShooter() && _player.delayIsReady(_frameRun))
			{
				...
				if (_currentTool == HOMING)
				{
					...
					{
						target = &*(_bats.begin());
						float closestEnemyDist = distanceBetweenMP(_player, *(_bats.begin()));

						for (Crawler& c : _bats)
						{
							float actualEnemyDist;

							if ((actualEnemyDist = distanceBetweenMP(_player, c)) < closestEnemyDist)
							{
								closestEnemyDist = actualEnemyDist;
								target = &c;
							}
						}
					}
